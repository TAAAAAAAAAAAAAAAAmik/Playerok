...
## Токен Playerok через бота

Токен сессии живёт недолго и слетает при смене IP, а раньше менялся только на
сервере: правка `.env` плюс перезапуск службы. Теперь его присылают боту:

```
/token
```

Бот попросит значение, проверит его запросом к аккаунту и покажет, чей он.
Годный — сохраняет и **начинает пользоваться сразу, без перезапуска**;
негодный — возвращает прежний, чтобы не остаться совсем без сессии. Сообщение
с токеном бот удаляет: история переписки живёт долго, а в ней ключ от аккаунта.

Формат любой: само значение куки, `token=eyJ…`, строка `Cookie` целиком или
выгрузка расширения вроде Cookie Editor в JSON. Из нескольких кук берётся
**только `token`** — остальные с чужого компьютера не просто бесполезны, а
вредны: `__ddg*` привязана к IP и User-Agent, и на сервере с другим адресом
она сразу сделала бы сессию недействительной. Свою куку защиты сервер получает
сам. Если `token` в присланном не нашёлся, бот скажет об этом и попросит ещё раз.

Команда работает только из чата, указанного в `TELEGRAM_CHAT_ID`.

Хранится он в `.playerok_cookie` (права 600, в репозиторий не попадает) и
перекрывает значение из `.env` — то остаётся запасным, начальным.

## Обновление

Тот же установщик служит обновлением: заполненные значения `.env` он не
переспрашивает, Chrome и библиотеки не трогает, если они на месте.

```bash
cd ~/playerok-bot && bash deploy_home.sh          # без root
cd /opt/playerok-bot && git pull && bash deploy.sh # с root
```

Запускать можно и из папки бота: скрипт лежит в самом репозитории, и
обновление кода перезаписало бы его прямо во время исполнения — bash дочитал
бы уже другой текст со старого смещения. Поэтому он сам переносит себя во
временный файл и продолжает оттуда.

## Установка без вопросов

Установщику можно передать значения переменными окружения — тогда он ничего
не спросит. Так удобнее и безопаснее: секреты не попадают ни в репозиторий,
ни в историю переписки.

```bash
TELEGRAM_BOT_TOKEN=123:AA… TELEGRAM_CHAT_ID=456… bash deploy_home.sh
```

Токен Playerok здесь не нужен — его спросит сам бот командой `/token`.
