...
Чтобы он поднимался сам — после падения и после перезагрузки сервера:

```bash
crontab -e
```

Одной строкой (подставьте свой путь):

```
* * * * * /home/tamik/playerok/templates/autodelivery/run_bot.sh
```

Cron будет звать скрипт раз в минуту; пока бот жив, вызов проходит впустую.
Упал — поднимется в течение минуты.
