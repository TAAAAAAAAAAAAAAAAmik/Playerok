...
браузера. Поэтому `/create` работает иначе — бот держит **открытый мастер**
в браузере (`wizard_session.py`), читает с его экрана пункты и показывает их
кнопками, а выбор пользователя превращает в клики.
...
Отсюда правило: **первый товар в связке создаётся браузером**, а дальше копии
идут **запросами** — `createItem` + `publishItem`, без Chrome, за секунды.
Если каталог связку не знает или запрос не прошёл, копия молча уходит на
браузерный путь, так что сценарий не ломается.

### Скидка

Цена вводится как обычно, а на том же шаге бот ищет поле скидки и ставит
`PLAYEROK_DISCOUNT` (по умолчанию 27%). Если у категории такого поля нет, шаг
не падает — в ответе будет «скидку задать не вышло», цена останется исходной.
