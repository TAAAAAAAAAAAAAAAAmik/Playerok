...
В запросах скидка уходит полем `input.discount` в `createItem`. Схема может его
не знать — тогда клиент видит это в тексте ошибки, повторяет запрос без поля и
создаёт товар по обычной цене, а не падает.
