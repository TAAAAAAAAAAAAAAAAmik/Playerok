...
`/create` спрашивает всё кнопками: списки игр, категорий, способов передачи и
характеристик тянутся из API, поэтому названия всегда совпадают с сайтом.
Игры листаются по 8 штук, есть поиск по названию. Текстом вводится только то,
что списком не предложишь: название, описание, значения полей товара и цена.

Выбранные пункты кладутся в `ProductDraft` уже идентификаторами
(`game_id`, `category_id`, `obtaining_type_id`, `attribute_values`,
`data_field_values`), так что `api_creator` не ищет их по названиям повторно.
Поиск по названию остаётся для случаев, когда черновик собран не кнопками:
