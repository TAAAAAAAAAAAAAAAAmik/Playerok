    ...
    while True:
        try:
            for order in await market.paid_orders():
                ...
                # Запоминаем ТОЛЬКО доведённое до конца. Пометив заказ до
                # выдачи, мы теряли его насовсем при любом случайном сбое:
                # каталог не прочитался, поставщик моргнул — и оплаченный
                # заказ больше не пробовался до перезапуска бота.
                #
                # Отказ пробуется каждым проходом и однажды проходит сам
                # (номинал вернулся в наличие, связь поднялась). Продавца
                # это не засыпает: про одну и ту же причину движок говорит
                # один раз.
