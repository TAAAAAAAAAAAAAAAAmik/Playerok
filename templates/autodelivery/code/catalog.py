...
class Card:
    ...
    # Как продавцы пишут единицу рядом с числом: «50 робуксов», «50 ROBUX».
    # По ней номинал читается из описания, когда в названии числа нет.
    # Только единица товара, не название игры: «Roblox 2024» — это год.
...
# валюта у кода зависит от РЕГИОНА, а не от карты.
MONEY_WORDS = (
    "$", "usd", "доллар", "долл", "€", "eur", "евро", "₽", "руб", "rub",
    "£", "gbp", "фунт", "₺", "try", "лир", "₹", "inr", "¥", "jpy", "cny",
    "zł", "pln", "kč", "czk", "kr", "sek", "nok", "dkk", "r$", "brl",
    "₩", "krw", "₪", "ils", "aed", "sar", "qar", "kwd", "₱", "php",
    "฿", "thb", "₫", "vnd", "rp", "idr", "rm", "myr", "chf", "mxn", "ars",
)


def unit_spellings(card: Card | None) -> tuple:
    """Как у этой карты пишут единицу рядом с числом."""
    if card is None:
        return ()

    if getattr(card, "unit", UNITS) == UNITS:
        words = [card.measure] + list(getattr(card, "measure_words", ()) or ())
...
def numbers_by_unit(text: str, words) -> list:
    """Числа, у которых рядом стоит единица: «50 робуксов» → [50].

    Рядом — значит вплотную или через пробел, с любой стороны: продавцы
    ...
    for word in words or ():
        ...
        # Единица должна начинаться со слова, а не с его середины:
        # «50 робуксов» — да, «50 микроробуксов» — нет. А вот хвост после
    ...
    # читает своё описание сверху вниз, и объяснение отказа должно
    # перечислять номиналы так же.
    out: list = []
...
# Как в описании выглядит ДИАПАЗОН, а не номинал: «от 50 до 10000
# робуксов», «50–10000». Такое встречается у продавцов, торгующих
...
def nominal_by_unit(text: str, card: Card | None) -> tuple:
    """Номинал из описания по единице → (номинал, причина отказа).

    Последняя попытка, и она нужна: продавец пишет номинал в описании
    обычными словами — «Вы получите 50 робуксов», — а помеченной строки
    «Номинал: 50» у объявления, сделанного руками, нет и взяться ей
    неоткуда.

    Число с единицей — не догадка «самое крупное», а прямое указание. Но
    если таких чисел несколько и они разные («от 50 до 10000 робуксов»),
    выбирать за продавца нельзя: это его деньги.
    """
    found = numbers_by_unit(text, unit_spellings(card))
    ...
    if looks_like_range(text):
        ...
        # границей, и взять её значит купить самый дорогой номинал из
        # названных. За продавца такое не решают.
        return None, ("в описании назван диапазон, а не номинал. Допишите "
                      "строку «Номинал: …» с тем числом, которое покупатель "
                      "получает по этому объявлению")

    if len(found) > 1:
        shown = ", ".join(shown_number(v) for v in sorted(found))

        return None, (f"в описании несколько номиналов ({shown}) — какой "
                      f"покупать, непонятно. Допишите строку «Номинал: "
                      f"{shown_number(sorted(found)[0])}»")

    return found[0], ""


def nominal_for(title: str, description: str = "", price=None,
                card: Card | None = None) -> tuple:
    """Сколько покупать → (номинал, причина отказа).

    Три источника, и они разные по надёжности:

    1. помеченная строка описания («Номинал: 50») — номинал СКАЗАН;
    2. число в названии — УГАДАН по самому крупному;
    3. число с единицей в описании («50 робуксов») — тоже сказан, но
       словами.

    Третий стоит последним не потому, что хуже второго, а потому, что
    появился позже и менять поведение работающих объявлений им нельзя.
    Он спасает ровно тот случай, где раньше был отказ: в названии числа
    нет вовсе — «🥳ПРОМОКОДОМ🥳 АВТОВЫДАЧА😎», — а в описании номинал
    написан по-человечески.
    ...
    # Ни строки, ни числа в названии. Ищем в описании число с единицей:
    # «50 робуксов», «10$».
    by_unit, why = nominal_by_unit(description, card)
    ...
    measure = (card.measure if card is not None else "") or ""
    example = f"«Номинал: 50» (столько {measure} в коде)" if measure \
        else "«Номинал: 50» — вместо 50 напишите, сколько в коде"

    # Показываем, что именно бот прочитал: без этого продавец смотрит на
    # своё описание, видит там число и не понимает, чем оно боту не
    # угодило. А не угодило тем, что рядом с ним нет единицы.
    seen = numbers_in(description)[:5]
    saw = (f" В описании вижу числа: "
           f"{', '.join(shown_number(v) for v in seen)} — но ни одного с "
           f"единицей рядом." if seen else "")

    return None, (f"номинал не найден: в названии товара нет числа, а в "
                  f"описании не нашлось ни строки «Номинал: …», ни числа с "
                  f"единицей ({measure or 'например «50 робуксов»'}).{saw} "
                  f"Допишите в описание строку вида {example}")
