...
# Как площадка говорит «сейчас нельзя». Это НЕ окончательный отказ:
# попытка была верной, просто её отвергли по темпу. Считать такое
# неудачей значит бросить товар навсегда из-за минутной заминки.
LATER = ("слишком много попыток", "too many", "429", "rate limit",
         "попробуйте повторить запрос позже", "попробуйте позже",
         "сбавить темп")


def try_later(why) -> bool:
    """Временный ли это отказ — из тех, что проходят сами."""
    low = str(why or "").lower()

    return any(word in low for word in LATER)
